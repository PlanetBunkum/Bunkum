...
using BunkumGeminiSampleApplication.Models;

namespace BunkumGeminiSampleApplication.Endpoints;

public class WeatherEndpoints : EndpointGroup
{
    [GeminiEndpoint("/api/v1/weather", ContentType.Json)]
    // This is a simple endpoint - the bare minimum.
    // The only requirement of an endpoint is that it is marked with an [GeminiEndpoint] attribute and the first argument is a RequestContext.
    ...
    [GeminiEndpoint("/api/v2/weather", ContentType.Json)]
    ...
    [GeminiEndpoint("/api/v3/weather", ContentType.Json)]
