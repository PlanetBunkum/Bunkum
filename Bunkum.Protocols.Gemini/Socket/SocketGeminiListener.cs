...
public partial class SocketGeminiListener : BunkumGeminiListener
{
    ...
    private async Task<ListenerContext?> ReadRequestIntoContext(System.Net.Sockets.Socket client, Stream rawStream)
    {
        ...
        if (uri.HostNameType is not UriHostNameType.IPv4 and not UriHostNameType.IPv6 && !this._config.AllowProxyRequests)
        {
            ...
            // If either port is -1 and the other is 1965,
            // because C# does not detect 1965 as the default port for Gemini requests, we have to manually do this check ourselves
            if ((uri.Port == 1965 && this._externalUri.Port == -1) ||
                (uri.Port == -1 && this._externalUri.Port == 1965))
            {
                if(!this._externalUri.Host.Equals(uri.Host, StringComparison.InvariantCultureIgnoreCase))
                    bad = true;
            }
